...
public class CheckRegisterDataService {
    ...
    /**
     * HIGH-LEVEL: Reads user check register with smart fallback and merging logic.
     * Pattern: Local user entries + Network team entries → Smart fallback → Merge
     * This is what users see when accessing their own check register (includes team lead reviews).
     *
     * @param username Username
     * @param userId User ID
     * @param year Year
     * @param month Month
     * @return Combined check register entries (user + team lead merged)
     */
    public List<RegisterCheckEntry> readUserCheckRegisterWithSmartFallbackAndMerge(String username, Integer userId, int year, int month) {
        try {
            // Step 1: Read user's own check register entries (local with smart fallback)
            List<RegisterCheckEntry> userEntries = readUserCheckRegisterLocalReadOnly(username, userId, year, month);

            // Step 2: Read team lead's review entries from network (no local fallback)
            List<RegisterCheckEntry> teamLeadEntries = readTeamLeadCheckRegisterFromNetworkOnly(username, userId, year, month);

            LoggerUtil.info(this.getClass(), String.format(
                    "User check register merge for %s - %d/%d: %d user entries + %d team lead entries",
                    username, year, month, userEntries.size(), teamLeadEntries.size()));

            // Step 3: Return both lists - let CheckRegisterService handle the merging logic
            // For now, we'll return user entries and let the service layer handle merging
            return userEntries;

        } catch (Exception e) {
            LoggerUtil.error(this.getClass(), String.format(
                    "Error reading user check register with merge for %s - %d/%d: %s",
                    username, year, month, e.getMessage()));
            return new ArrayList<>();
        }
    }
    ...
    /**
     * HIGH-LEVEL: Reads team lead check register with smart fallback and cross-reference logic.
     * Pattern: Local team entries + Network user entries → Smart fallback → Cross-reference
     * This is what team leads see when accessing a user's check register for review.
     *
     * @param username Username (target user being reviewed)
     * @param userId User ID
     * @param year Year
     * @param month Month
     * @return Team lead check register entries with user context
     */
    public List<RegisterCheckEntry> readTeamLeadCheckRegisterWithSmartFallbackAndContext(String username, Integer userId, int year, int month) {
        try {
            // Step 1: Read team lead's review entries (local with smart fallback)
            List<RegisterCheckEntry> teamLeadEntries = readTeamLeadCheckRegisterLocalReadOnly(username, userId, year, month);

            // Step 2: Read user's original entries from network for context (no local fallback)
            List<RegisterCheckEntry> userEntries = readUserCheckRegisterFromNetworkOnly(username, userId, year, month);

            LoggerUtil.info(this.getClass(), String.format(
                    "Team lead check register context for user %s - %d/%d: %d team lead entries + %d user entries context",
                    username, year, month, teamLeadEntries.size(), userEntries.size()));

            // Return team lead entries - let CheckRegisterService handle any context merging
            return teamLeadEntries;

        } catch (Exception e) {
            LoggerUtil.error(this.getClass(), String.format(
                    "Error reading team lead check register with context for user %s - %d/%d: %s",
                    username, year, month, e.getMessage()));
            return new ArrayList<>();
        }
    }
