...
public class TeamCheckRegisterController extends BaseController {
     ...
     * POST /team/check-register/upload-excel
     */
    ...
    public String uploadExcelFile(
            ...
            @RequestParam Integer year,
            @RequestParam Integer month,
            @RequestParam String adminSync,
            RedirectAttributes redirectAttributes) {

        try {
            LoggerUtil.info(this.getClass(), String.format(
                    "Uploading Excel file for user %s, year: %d, month: %d, adminSync: %s",
                    username, year, month, adminSync));

            // Get current team lead user
            User teamLeader = getUserService().getUserByUsername(userDetails.getUsername()).orElse(null);
            if (teamLeader == null) {
                redirectAttributes.addFlashAttribute("errorMessage", "Team leader user not found");
                return getRedirectUrl(username, userId, year, month);
            }
            ...
            ServiceResult<List<RegisterCheckEntry>> parseResult = excelProcessingService.processExcelFile(
                    file, username, userId, teamLeader.getUsername(), adminSync);
            ...
            // Save entries to team check register
            int successCount = 0;
            int errorCount = 0;
            ...
            for (RegisterCheckEntry entry : entries) {
                try {
                    ServiceResult<RegisterCheckEntry> saveResult = checkRegisterService.updateTeamEntry(
                            username, userId, entry, year, month);
