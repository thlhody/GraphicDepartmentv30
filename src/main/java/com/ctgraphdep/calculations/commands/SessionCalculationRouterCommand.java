 ...
 */
public class SessionCalculationRouterCommand extends BaseSessionCalculationsCommand<WorkUsersSessionsStates> {
     ...
     */
    public SessionCalculationRouterCommand(WorkUsersSessionsStates session, LocalDateTime currentTime, int userSchedule) {
    ...
    protected WorkUsersSessionsStates executeCommand(CalculationContext context) {
        ...
        if (WorkCode.WORK_ONLINE.equals(session.getSessionStatus())) {
            ...
            context.executeCommand(context.getCommandFactory().createUpdateOnlineSessionCalculationsCommand(session, currentTime, userSchedule));
        } else if (WorkCode.WORK_TEMPORARY_STOP.equals(session.getSessionStatus())) {
            context.executeCommand(context.getCommandFactory().createUpdateTempStopCalculationsCommand(session, currentTime));
        }
