 ...
 * WorkTimeDisplay.js
 *
 * Handles cell display updates and special day work formatting.
 * Manages SN overtime display, work time calculations, and cell content formatting.
 *
 * @module features/time-management/WorkTimeDisplay
 */
 ...
 * Manages display formatting for work time cells
 */
export class WorkTimeDisplay {
     ...
     */
    static initialize() {
        console.log('Initializing Work Time Display...');

        this.initializeSNOvertimeDisplay();
        this.initializeSpecialDayTooltips();

        console.log('✅ Work Time Display initialized');
    }
     ...
     */
    static updateCellDisplay(cell, field, value, rawData = null) {
     ...
     */
    static updateTempStopDisplay(cellValue, value) {
     ...
     */
    static updateTimeOffDisplay(cellValue, value, rawData) {
        if (value && value !== '-') {
            ...
            if (isSnWithOvertime) {
                ...
                cellValue.innerHTML = `<span class="sn-work-display" title="National Holiday with ${TimeManagementUtilities.formatMinutesToReadable(rawData.totalOvertimeMinutes)} overtime work">${snDisplay}</span>`;
                ...
                const timeOffClass = TimeManagementUtilities.getTimeOffClass(value);
                const timeOffLabel = TimeManagementUtilities.getTimeOffLabel(value);
     ...
     */
    static updateWorkTimeDisplay(cellValue, value, rawData) {
     ...
     */
    static updateOvertimeDisplay(cellValue, value, rawData) {
        if (value && value !== '-') {
            ...
            if (isSnOvertime) {
                ...
                title = `National Holiday work: ${value}`;
     ...
     */
    static updateTimeDisplay(cellValue, value, rawData) {
        // Format the time properly for display
        let displayTime = '-';

        if (value) {
            // Ensure consistent 24-hour format for display
            displayTime = TimeInput.formatTime(value, '24hour');
        }
     ...
     */
    static updateDefaultDisplay(cellValue, value) {
     ...
     */
    static initializeSNOvertimeDisplay() {
     ...
     */
    static applySpecialDayWorkStyling(row, rowData) {
     ...
     */
    static initializeSpecialDayTooltips() {
        ...
        specialDaySelectors.forEach(selector => {
            document.querySelectorAll(selector).forEach(element => {
                ...
                const type = TimeManagementUtilities.getSpecialDayTypeFromClass(element.className);
                const hours = TimeManagementUtilities.extractHoursFromDisplay(element.textContent);
                const tooltip = TimeManagementUtilities.generateSpecialDayTooltip(type, hours);
        ...
        TimeManagementUtilities.initializeTooltips();
    }
    ...
    static getRowData(row) {
        ...
        const overtimeMinutes = row.getAttribute('data-overtime-minutes') ||
            (row.querySelector('.overtime-cell') ?
                TimeManagementUtilities.extractOvertimeMinutes(row.querySelector('.overtime-cell').textContent) : 0);
     ...
     */
    static refreshRowAfterUpdate(row, updatedData) {
        ...
        if (this.isSpecialDayWithWork(updatedData)) {
            ...
            if (overtimeCell && updatedData.totalOvertimeMinutes) {
                const formattedOvertime = TimeManagementUtilities.formatMinutesToReadable(updatedData.totalOvertimeMinutes);
     ...
     */
    static showSNWorkNotification(actionType) {
        const messages = {
     ...
     */
    static validateFieldForSpecialDay(field, value, rowData) {
     ...
     */
    static async checkCellEditability(cell) {
     ...
     */
    static refreshSpecialDayDisplays() {
        this.initializeSNOvertimeDisplay();
        this.initializeSpecialDayTooltips();
    }
     ...
     */
    static updateRowCells(row, updates) {
     ...
     */
    static getFormattedDisplayValue(field, value, context = {}) {
        switch (field) {
            case 'tempStop':
            ...
            case 'startTime':
            case 'endTime':
                return TimeInput.formatTime(value, '24hour') || '-';
            case 'overtime':
                return TimeManagementUtilities.formatMinutesToReadable(parseInt(value) || 0);
            default:
                return value || '-';
        }
    }
