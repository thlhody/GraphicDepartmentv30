 ...
 * TimeInput.js
 *
 * Handles all 24-hour time input functionality.
 * Provides consistent 24-hour time input with auto-formatting,
 * validation, and visual feedback across the application.
 *
 * @module features/time-management/TimeInput
 */

/**
 * TimeInput class
 * Static class for creating and managing 24-hour time inputs
 */
export class TimeInput {
     ...
     */
    static create24HourEditor(currentValue = '', options = {}) {
     ...
     * @private
     */
    static _setupAttributes(editor, config) {
     ...
     * @private
     */
    static _setupEventListeners(editor, config) {
     ...
     * @private
     */
    static _setupStyling(editor, config) {
     ...
     * @private
     */
    static _handleInput(event, config) {
        let value = event.target.value.replace(/[^\d]/g, ''); // Remove non-digits

        // Auto-formatting with colon
        if (value.length >= 3) {
            if (value.length === 3) {
     ...
     * @private
     */
    static _handlePaste(event, config) {
     ...
     * @private
     */
    static _handleBlur(event, config) {
     ...
     * @private
     */
    static _updateVisualFeedback(input, value, config) {
     ...
     * @private
     */
    static _attemptAutoCorrection(value) {
     ...
     */
    static getValidationError(timeString) {
     ...
     * @private
     */
    static _convertDigitsToTime(digits) {
        if (digits.length === 3) {
     ...
     * @private
     */
    static _convert12HourTo24Hour(match) {
     ...
     * @private
     */
    static _handlePartialInput(hoursString) {
     ...
     */
    static formatTime(timeString, format = '24hour') {
        if (!this.validateTime(timeString)) {
            return timeString;
        }
     ...
     * @private
     */
    static _convertTo12Hour(timeString) {
     ...
     */
    static createHelpText(fieldType = 'time') {
